...
function hedonischerAlgorithmus(data, geo, infra, geodienste, marktPreis, steuerfussData) {
  ...
  // Wenn Live-Marktdaten verfügbar → nehmen. Sonst Fallback-Tabelle.
  let base;
  if (marktPreis?.marktpreisM2 && marktPreis.marktpreisM2 > 3000) {
    // Live-Daten haben Vorrang
    base = marktPreis.marktpreisM2;
    console.log(`Basispreis von Live-API: CHF ${base}/m²`);
  } else {
    // Fallback-Tabelle (Schweizer PLZ-Regionen)
    if      (plz.startsWith('87') || plz.startsWith('88')) base = 11500;
    else if (plz.startsWith('80') || plz.startsWith('81')) base = 12800;
    else if (plz === '6300' || plz === '6301' || plz === '6302') base = 14000;
    ...
    console.log(`Basispreis aus Fallback-Tabelle: CHF ${base}/m²`);
  }
