...
module.exports = async function handler(req, res) {
  ...
  // 1. Live-Geodaten sammeln (Nominatim, Overpass, Swisstopo)
  const geoResult = await analysiereImmobilie(body);
  console.log(`Geodaten: ${geoResult.distBahnhofM}m Bahn | ${geoResult.distSeeM}m See | Steuerfuss ${geoResult.steuerfuss}`);

  // 2. KI-Agent: Claude bewertet anhand aller Geodaten
  const result = await bewerteWithAgent(body, geoResult, geoResult);
  console.log(`Marktwert: CHF ${fmt(result.total)} | Score: ${result.score} | Quelle: ${result.quelle}`);
