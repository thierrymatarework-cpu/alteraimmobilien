...
// PDFKit — kein Browser, läuft zuverlässig auf Vercel
async function generatePDF(data, result) {
  return await buildPDF(data, result);
}
...
<table width="600" cellpadding="0" cellspacing="0" border="0"
  ...
  <tr><td style="padding:20px 36px 0">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="background:#EDF1EE;border:1.5px solid #D0DDD4;border-radius:10px">
    <tr><td style="padding:16px 20px">
      <table width="100%" cellpadding="0" cellspacing="0" border="0"><tr>
        <td style="font-size:24px;padding-right:14px;vertical-align:middle">📄</td>
        <td>
          <div style="font-size:13px;font-weight:700;color:#344E41;margin-bottom:3px">PDF-Report im Anhang dieser E-Mail</div>
          <div style="font-size:12px;color:#555;line-height:1.6">
            <strong>Altera-Bewertungsreport-${plz}.pdf</strong> ist dieser E-Mail angehängt.<br>
            Enthält: Objektdaten · Score-Analyse · Marktvergleich · Standort · Einschätzung<br>
            <span style="color:#888">PDF öffnen → speichern oder drucken</span>
          </div>
        </td>
      </tr></table>
    </td></tr>
  ...
  <tr><td style="padding:20px 36px 0">
    <table width="100%" cellpadding="0" cellspacing="0" border="0"
           style="background:#FBF3E8;border:1.5px solid #E8D9B8;border-radius:10px">
    <tr><td style="padding:14px 18px;font-size:12px;color:#966B1A">
      ⚠️ Der PDF-Report konnte nicht automatisch generiert werden. Bitte kontaktieren Sie uns direkt unter hallo@altera-immobilien.ch.
    </td></tr></table>
  </td></tr>`}
  ...
  <tr><td style="padding:20px 36px 0">
    ...
    <tr><td style="padding:22px 26px">
      <div style="font-size:15px;font-weight:700;color:#fff;margin-bottom:6px">Professionelle Schätzung vor Ort</div>
      <div style="font-size:13px;color:rgba(255,255,255,0.6);margin-bottom:16px;line-height:1.6">
        Thierry oder Janis kommen persönlich zu Ihnen. Bankfähig, unverbindlich.
      </div>
...
module.exports = async function handler(req, res) {
  ...
  // 2. PDF generieren mit PDFKit
  let pdfBuffer = null;
  try {
    console.log('PDF wird generiert...');
    pdfBuffer = await generatePDF(body, result);
