...
const { buildReportHTML, fmt } = require('../lib/report');
...
// ─── PDF generieren mit Puppeteer + Chromium ────────────────────────────────
async function generatePDF(html) {
  const chromium = require('@sparticuz/chromium-min');
  const puppeteer = require('puppeteer-core');

  // Chromium von CDN laden — kein lokales Binary nötig
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    const reportHTML = buildReportHTML(body, result);
    ...
    // Weiter ohne PDF — E-Mail wird trotzdem gesendet
  }
