...
// ─────────────────────────────────────────────────────────────────────────────
// Modularer Live-Marktdaten-Algorithmus — nutzt preisTabelle.js als Fallback
// Fragt echte öffentliche APIs ab, berechnet alle Kennzahlen
// und gibt ein vollständiges Bewertungsobjekt zurück
// ─────────────────────────────────────────────────────────────────────────────

const https = require('https');
const { getBasePreis, getSteuerfuss } = require('./preisTabelle');
...
async function fetchSteuerfuss(plz, ort) {
  const sf = getSteuerfuss(plz);
  const quelle = sf ? 'Preistabelle' : 'Schätzung';
  return { steuerfuss: sf || 110, steuerfussQuelle: quelle };
}
...
function hedonischerAlgorithmus(data, geo, infra, geodienste, marktPreis, steuerfussData) {
  ...
  // ── A. Basispreis ────────────────────────────────────────────────────────
  // Live-Marktdaten (Homegate/ImmoScout) oder preisTabelle.js (600+ Gemeinden, Stand 2026)
  let base;
  if (marktPreis?.marktpreisM2 && marktPreis.marktpreisM2 > 3000) {
    base = marktPreis.marktpreisM2;
    console.log(`Basispreis Live-API: CHF ${base}/m²`);
  } else {
    const preisInfo = getBasePreis(plz, data.typ);
    base = preisInfo.base;
    console.log(`Basispreis Tabelle: CHF ${base}/m² (ETW: ${preisInfo.etwM2}, Kanton: ${preisInfo.kanton})`);
  }
